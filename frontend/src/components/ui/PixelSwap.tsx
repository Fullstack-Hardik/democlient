...
import React, { useCallback, useEffect, useMemo, useRef, useState, CSSProperties, ReactNode } from 'react';
import './PixelSwap.css';

const MAX_PIXELS = 220;
const KEYFRAME_STEPS = 14;

const PATTERNS: Record<string, (x: number, y: number) => number | null> = {
  random: () => null,
...
const buildGrid = ({ width, height, pixelSize, gap, pattern, randomness }: any) => {
  let size = pixelSize;
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      pixels.push({
        ...
        offset: base === null ? random : (base as number) * (1 - mix) + random * mix
      });
    }
  }
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade }: any) => {
  const window = [];
  const content = [];
...
export default function PixelSwap({
  ...
  useEffect(() => {
    ...
    frozenGrid.pixels.forEach((pixel: any, index: number) => {
  ...
  const interactionProps: any = useMemo(() => {
    if (trigger === 'hover') {
      return {
    ...
    >
      ...
      {transition && (
        <div className="pixel-swap__grid" aria-hidden="true">
          {transition.grid.pixels.map((pixel: any, index: number) => (
            <div
              key={pixel.id}
              ref={element => {
